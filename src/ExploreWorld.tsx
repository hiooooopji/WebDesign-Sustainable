/**
 * ═══════════════════════════════════════════════════════════════════
 *  BOILERPLATE — CSS3D Explore World
 * ═══════════════════════════════════════════════════════════════════
 *
 *  SETUP (2 min):
 *    npm create vite@latest my-app -- --template react-ts
 *    cd my-app && npm i three
 *    npm i -D @types/three
 *    # drop this file as src/ExploreWorld.tsx
 *    # put .html pages in public/
 *    # set App.tsx to: <ExploreWorld />
 *
 *  CUSTOMIZE (3 min):
 *    1. Edit SCREENS array below (label, url, color)
 *    2. Adjust CAM_START / CAM_TARGET for initial view
 *    3. Tweak STAR_COUNT, SENSITIVITY as needed
 *
 *  HOW IT WORKS:
 *    - CSS3DRenderer renders real DOM elements in 3D space
 *    - Pointer lock → mouse controls camera rotation
 *    - Dot-product check highlights which screen you're looking at
 *    - Click a screen → overlay iframe pops up
 *    - Click backdrop / ✕ → close overlay, re-lock mouse
 *
 * ═══════════════════════════════════════════════════════════════════
 */
...
// ─── CONFIG ──────────────────────────────────────────────────────
//  ↳ Change these to customize your world
...
const CAM_START   = [0, 320, 850] as const;  // initial camera position
const CAM_TARGET  = [0, -300, -100] as const; // what camera looks at
const SENSITIVITY = 0.002;                     // mouse look speed
const SPREAD      = 1.6;                       // angular spread of screens (radians)
const SCREEN_DIST = 1150;                      // how far screens are from center
const STAR_COUNT  = 250;                       // number of stars
...
const HL = {
  b: "2px solid rgba(16,185,129,0.8)",
  s: "0 0 50px rgba(16,185,129,0.3),0 0 120px rgba(16,185,129,0.1),0 40px 100px rgba(0,0,0,0.8)",
};
const NM = {
  b: "2px solid rgba(255,255,255,0.06)",
  s: "0 4px 60px rgba(0,0,0,0.6),0 0 80px rgba(16,185,129,0.03)",
};
...
/** Create a CSS3DObject from a styled div */
...
export default function ExploreWorld() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < STAR_COUNT; i++) {
      ...
      el.style.cssText =
        ...
        `box-shadow:0 0 ${sz*2}px rgba(${r},${g},${b},${a}),` +
        `0 0 ${sz*6}px rgba(${r},${g},${b},${a*.6}),` +
        `0 0 ${sz*14}px rgba(${r},${g},${b},${a*.2});`;
      const obj = new CSS3DObject(el);
    ...
    const floor = makeObj(
      `width:4000px;height:4000px;background-image:` +
      `linear-gradient(rgba(16,185,129,0.06) 1px,transparent 1px),` +
      `linear-gradient(90deg,rgba(16,185,129,0.06) 1px,transparent 1px);` +
      `background-size:80px 80px;mask-image:${GM};-webkit-mask-image:${GM};`,
      [0, -320, 0],
    );
    ...
    scene.add(makeObj(
      ...
      `background:radial-gradient(ellipse at center,rgba(16,185,129,0.04) 0%,rgba(16,185,129,0.01) 40%,transparent 70%);`,
      [0, -50, -400],
    ));
    ...
    for (let i = 0; i < SCREENS.length; i++) {
      ...
      lbl.style.cssText =
        ...
        `font-family:system-ui;font-size:14px;font-weight:600;letter-spacing:2px;` +
        `text-transform:uppercase;color:${s.color};opacity:0.7;white-space:nowrap;` +
        `text-shadow:0 0 20px ${s.color}44;`;
    ...
    const hl = (i: number) => {
      if (i === active) return;
      if (active >= 0) { iframes[active].style.border = NM.b; iframes[active].style.boxShadow = NM.s; }
      active = i;
      if (i >= 0) { iframes[i].style.border = HL.b; iframes[i].style.boxShadow = HL.s; }
    };
    ...
    const loop = () => {
      ...
      // Highlight screen you're looking at
  ...
  return (
    <div
      ...
      style={{ width: "100vw", height: "100vh", overflow: "hidden", position: "relative", cursor: "crosshair",
        background: "radial-gradient(ellipse at 50% 40%,#0a0f1e 0%,#020208 60%,#000002 100%)" }}
    >
      {/* Keyframes */}
      ...
      {/* HUD — title */}
      <div style={{ position:"absolute", top:28, left:0, width:"100%", textAlign:"center", zIndex:10,
        pointerEvents:"none", fontFamily:"system-ui" }}>
        <div style={{ fontSize:"0.75rem", fontWeight:600, color:"rgba(16,185,129,0.5)",
      ...
      {/* HUD — hint */}
      <div ref={hintRef} style={{ position:"absolute", bottom:40, left:0, width:"100%", textAlign:"center",
        zIndex:10, pointerEvents:"none", fontFamily:"system-ui", transition:"opacity 0.4s ease" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:12, background:"rgba(0,0,0,0.5)",
          backdropFilter:"blur(8px)", padding:"10px 24px", borderRadius:9999,
          border:"1px solid rgba(255,255,255,0.06)" }}>
          <span style={{ color:"rgba(255,255,255,0.3)", fontSize:"0.8rem" }}>
            Click to lock · Look around · Click screen to open
          </span>
        </div>
      </div>
      ...
      <div style={{ position:"absolute", bottom:0, left:0, width:"100%", height:"40%", zIndex:1,
        pointerEvents:"none", background:"linear-gradient(to top,rgba(16,185,129,0.03) 0%,transparent 100%)" }} />

      {/* Overlay */}
      {overlay && (
        <div onClick={closeOverlay} style={{ position:"fixed", inset:0, zIndex:100,
          display:"flex", alignItems:"center", justifyContent:"center",
          background:"rgba(0,0,0,0.75)", backdropFilter:"blur(6px)", cursor:"pointer", animation:"fi 0.2s ease" }}>
          <div onClick={e => e.stopPropagation()} style={{ width:"90vw", maxWidth:1200, height:"85vh",
            position:"relative", borderRadius:20, overflow:"hidden",
            border:`2px solid ${overlay.color}88`, cursor:"default",
            boxShadow:`0 0 60px ${overlay.color}33,0 20px 80px rgba(0,0,0,0.8)`, animation:"pi 0.25s ease" }}>
            <button onClick={closeOverlay} style={{ position:"absolute", top:16, right:16, zIndex:10,
              width:40, height:40, borderRadius:"50%", border:"1px solid rgba(255,255,255,0.15)",
              background:"rgba(0,0,0,0.6)", color:"rgba(255,255,255,0.7)", fontSize:"1.2rem",
              cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
            <div style={{ position:"absolute", bottom:0, left:0, width:"100%", padding:"12px 20px",
              background:"linear-gradient(to top,rgba(0,0,0,0.7),transparent)", fontFamily:"system-ui",
              fontSize:"0.85rem", fontWeight:600, letterSpacing:"2px", textTransform:"uppercase",
              color:overlay.color, textAlign:"center" }}>{overlay.label}</div>
